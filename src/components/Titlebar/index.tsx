...
export default class Index extends Component<Props, State> {
  ...
  componentDidMount = () => {
    this.currentWindow.addListener('maximize', () =>
      this.setState({ isMaximized: true })
    );
    this.currentWindow.addListener('unmaximize', () =>
  ...
  renderWindows() {
    ...
    return (
      <Controls key="title-controls">
        <ButtonWindows aria-label="minimize" onClick={this.handleMinimize}>
          <svg version="1.1" aria-hidden="true" width="10" height="10">
            <path d="M 0,5 10,5 10,6 0,6 Z" />
          </svg>
        </ButtonWindows>
        <ButtonWindows aria-label="maximize" onClick={this.handleMaximize}>
          {isMaximized ? (
            <svg version="1.1" aria-hidden="true" width="10" height="10">
        ...
        <CloseButtonWindows aria-label="close" onClick={this.handleClose}>
  ...
  renderMacOS() {
    return (
      <Controls key="title-controls">
        <ButtonMacClose onClick={this.handleClose}>
        ...
        <ButtonMacMinimize onClick={this.handleMinimize}>
  ...
  render() {
    ...
    if (this.isWindows) {
      elements.push(
        <Text key="title-text" isWindows={this.isWindows}>
          {title}
        </Text>
      );
